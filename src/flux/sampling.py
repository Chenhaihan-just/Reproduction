...
from .model import ContextualRepulsionConfig, Flux
...
def denoise(
    ...
    repulsion_steps = 0 if contextual_repulsion is None else len(timesteps) - 1
    if contextual_repulsion_tau is not None:
        repulsion_steps = min(repulsion_steps, contextual_repulsion_tau)
    ...
    for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:])):
        ...
        if contextual_repulsion is not None and step_idx < repulsion_steps:
            model_kwargs["contextual_repulsion"] = contextual_repulsion
